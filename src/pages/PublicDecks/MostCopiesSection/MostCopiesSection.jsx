...
function MostCopiesSection() {
    ...
    return (
        <section className="mb-16">
            ...
            {displayLimit < MAX_DECKS ? (
                <div className="mt-6 text-center">
                    <button
                        className="text-purple-400 hover:text-purple-300 font-medium transition-colors duration-200"
                        onClick={() => setDisplayLimit(prev => Math.min(prev + 3, MAX_DECKS))}
                    >
                        <div className='flex items-center gap-2'>
                            <span>
                                {displayLimit === 3 ? 'View more popular decks' : 'Show more'}
                                {displayLimit > 3 && ` (${Math.min(3, MAX_DECKS - displayLimit)} more)`}
                            </span>
            ...
            {/* Show Less Button (only when showing more than 3) */}
            {displayLimit > 3 && (
                <div className="mt-3 text-center">
                    <button
                        className="text-slate-500 hover:text-slate-400 font-medium transition-colors duration-200 text-sm"
