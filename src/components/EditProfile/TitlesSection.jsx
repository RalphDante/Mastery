...
import { PROGRESSION_TITLES, SPECIAL_TITLES, LOOT_TITLES } from "../../configs/titlesConfig";
...
export default function TitlesSection({ selectedTitle, setSelectedTitle }) {
  ...
  // Special titles (manually granted)
  ...
  // Loot titles (obtained from loot boxes)
  const unlockedLootTitles = (user?.unlockedTitles || [])
    .map(key => {
      // Check all rarity tiers for this title
      for (const rarity of ['uncommon', 'rare', 'legendary']) {
        const lootTitle = LOOT_TITLES[rarity]?.find(t => t.id === key);
        if (lootTitle) {
          return {
            ...lootTitle,
            subtitle: `${lootTitle.rarity.charAt(0).toUpperCase() + lootTitle.rarity.slice(1)} Loot`,
            isLoot: true,
            unlocked: true,
          };
        }
  ...
  const handleTitleClick = (title) => {
    ...
    // Allow selection of unlocked titles
  ...
  const displayedTitleObj = titles.find(t => t.id === selectedTitle);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-800/50 rounded-xl p-6 text-center border border-slate-700">
        ...
        >
          {displayedTitleObj?.title || currentRankTitle}
        </p>
      </div>
          ...
          >
            ...
            <div className="relative text-center">
              ...
              <p className="text-slate-300 mb-6 text-sm sm:text-base">
                Get access to exclusive titles and premium features with Pro!
              </p>
